...
export class AgentLLMClient extends LLMClient {
  ...
  private async callGeminiAPI<T>(options: CreateChatCompletionOptions["options"], schema: any): Promise<T> {
  ...
  private async callWorkersAI<T>(options: CreateChatCompletionOptions["options"], schema: any): Promise<T> {
    if (!this.binding) {
    ...
    const result = await this.binding.run(this.modelName, {
