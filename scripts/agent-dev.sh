...
# "agent-swarm-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "agent-swarm-dev" is overwritten by
...
AGENT_NAME="agent-swarm-dev"
WORKSPACE="/workspaces/agent-swarm"
...
if [ -x "/workspaces/agent-swarm/scripts/agent-dev.sh" ]; then
  "/workspaces/agent-swarm/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
