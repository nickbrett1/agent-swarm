...
if command -v doppler &> /dev/null && doppler whoami &> /dev/null; then
  ...
  if doppler secrets --project agent-swarm --config dev --json 2>/dev/null | grep -q '"CLOUDFLARE_API_TOKEN"'; then
    echo "✅ Found CLOUDFLARE_API_TOKEN in Doppler. Running wrangler dev via Doppler..."
    exec doppler run --project agent-swarm --config dev -- npx wrangler dev "$@"
  fi
fi
